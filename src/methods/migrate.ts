...
export default async function migrate(
  ...
  let [migration, migrationBump] = await web3.PublicKey.findProgramAddress(
  ...
  try {
    ...
    step1Res = await web3.sendAndConfirmRawTransaction(
      this.connection,
      signedTx.serialize(),
  ...
  try {
    ...
    step2Res = await web3.sendAndConfirmRawTransaction(
      this.connection,
      signedTx.serialize(),
      { skipPreflight: false, commitment: "confirmed" }
    );
