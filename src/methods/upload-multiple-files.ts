...
export default async function uploadMultipleFiles(
    ...
    if (!isBrowser) {
        ...
        data.forEach(async (shdwFile: ShadowFile) => {
            ...
            const url = encodeURI(
                `https://shdw-drive.genesysgo.net/${key.toString()}/${
                    shdwFile.name
                }`
            );
  ...
  */
    ...
    if (fileData.length) {
        ...
        for (let chunkIdx = 0; chunkIdx < fileData.length; chunkIdx++) {
            ...
            const namesLength = Buffer.byteLength(
                Buffer.from(fileNames.join())
            );
                        ...
                        async (items) => {
                            ...
                            if (!response.ok) {
                                ...
                                if (responseJson.upload_errors.length) {
                                    // TODO add type here
                                    return responseJson.upload_errors.map(
                                        (error: any) => ({
                                            fileName: error.file,
